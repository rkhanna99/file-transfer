...
def copy_files(source_folder: str, destination_folder: str, max_workers: int=4, file_type: str=None, start_date: datetime=None, end_date: datetime=None) -> None:
    ...
    # Files to transfer
    # if file_type != None and cutoff_date == None:
    #     transfer_files = [f for f in os.listdir(source_folder) if f.lower().endswith(file_type)]
    # elif file_type != None and cutoff_date != None:
    #     transfer_files = [f for f in os.listdir(source_folder) if (f.lower().endswith(file_type) and (datetime.fromtimestamp(os.path.getmtime(os.path.join(source_folder, f))) <= cutoff_date))]
    # elif file_type == None and cutoff_date != None:
    #     transfer_files = [f for f in os.listdir(source_folder) if (datetime.fromtimestamp(os.path.getmtime(os.path.join(source_folder, f))) <= cutoff_date)]
    # else:
